...
where the U (N 2) hits the lower-right-hand corner and doesn’t see our solution, while the U (2) is included in the denominator because it acts like V in the original solution (2.51) and we don’t want to over count. The notation S[U (p) ⇥ U (q)] means that we lose the overall central U (1) ⇢ U (p) ⇥ U (q). 

The coset space above has dimension 4N 8. This means that the solution in which (2.51) is embedded into SU (N ) comes with 4N collective coordinate. This is the most general ⌫ = 1 instanton solution in
SU (N ).
