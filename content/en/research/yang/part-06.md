...
We can also study the quantum mechanical action (2.17) using the path integral. Here we fix the background gauge field Aμ and integrate only over the colour degrees of freedom w(⌧ ) and the Lagrange multiplier (⌧ ). 
...
Then we can consider gauge transformations ↵(⌧ ) in which ↵ winds around R the circle, so that d⌧ ↵ ̇ = 2⇡n for some n 2 Z. The action (2.17) would then change
...
the dotted lines represent insertions of the gauge fields A. 
...
This, of course, is the Wilson loop W [C]. We see that we get a slightly di↵erent perspective on the Wilson loop: it arises by integrating out the colour degrees of freedom of the quark test particle.
...
Note, however, that the canonical normalisation of the Yang-Mills theta term di↵ers by a factor of 12 from the Maxwell term (a fact which is a little hidden in this notation because it’s buried in the definition of the trace (2.2)).
...
This is similar in spirit to the kind of argument we saw in Section 1.2.4 for the U (1) theta angle, although the details differ because non-Abelian gauge groups have a di↵erent topology from their Abelian cousins. 

We will explain this in the rest of this section and, from a slightly di↵erent perspective, in Section 2.3.
...
This, it turns out, affects the spectrum of states in the Yang-Mills theory even when ✓ is constant. 

The purpose of this section is to explore this physics.
