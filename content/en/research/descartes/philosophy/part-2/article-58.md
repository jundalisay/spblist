...
It will move with the same speed as the force from the different fluid propels it, assuming that in this fluid there are no particles that move slower or at least equally as fast. 

If any move slower, then, as far as they are concerned, they don't account for the fluid's behavior, and then the slightest force is not enough to move the hard body existing in this fluid, but rather a force is required that exceeds the resistance arising from the tardiness of these fluid particles. 
...
This body B cannot move faster than it is propelled by an external force; although often all the particles of fluid FD have much more agitation. 
...
Thus, assuming that the hard body B, initially immobile in the midst of fluid FD, is now propelled by some external force, for example, by my hand with a slow motion, the sole impetus from my hand should not be considered a reason for it to move faster than it is propelled. 

Although all the fluid particles move much faster, it should not be assumed that they are determined to move in circular motions or similar ones that are faster than this impetus, but rather they are propelled towards any other parts as before, only faster.
