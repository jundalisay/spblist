...
weight: 46
description: "How can it be determined how much the motion of each body is changed because of the collision of other bodies? and that by the following rules"
writer:
  name: Rene Descartes
...
Assume Body B and C, were perfectly equal and moving at equal speeds
...
If B were just slightly larger than C, then only C would be reflected.
...
If body C were completely at rest and slightly larger than B, no matter how fast B moved towards C, it would never move C.
...
If the stationary body C were smaller than B, then no matter how slowly B moved towards C, it would move C with it, transferring a portion of its own motion to it, so that both would subsequently move at the same speed. 
...
These excesses are calculated as follows: If C were twice as large as B and B did not move twice as fast as C, it would not propel C but would be reflected in the opposite direction. However, if it moved more than twice as fast, it would propel C. 

For instance, if C had only two units of speed and B had five, subtracting two units from B would transfer only one unit to C, making both bodies move with three units of speed thereafter, because C is twice as large as B. 

This would result in both bodies, B and C, moving with three units of speed. This reasoning applies similarly to other cases. 
